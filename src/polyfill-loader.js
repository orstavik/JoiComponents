...
(function () {
  ...
  window.polyfill = {
    ...
    runWhenReady: function (pf) {       //empties the que and returns a promise resolved when all is run
      ...
      var q = que;
      que = undefined;
      let p = Promise.all(q.map(function (fn) {
        return fn instanceof Function ? fn() : fn;
      }));
      p.then(function () {
        document.dispatchEvent(new CustomEvent("polyfill-ready", {bubbles: true}))
      });
      return p;
    },
    await: function (pf) {
...
(function () {
  ...
  //   document.readyState === "loading" ?
  //     window.addEventListener("DOMContentLoaded", window.polyfill.runWhenReady) :
  //     window.polyfill.runWhenReady();
  // })();
})();