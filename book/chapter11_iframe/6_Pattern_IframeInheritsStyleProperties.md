# Pattern: IframeInheritsStyleProperties

The IframeInheritsStyleProperties pattern embeds an *untrusted* HTML fragment that *inherits* all inherited 
...
IframeInheritsStyleProperties is implemented as a web component that uses the StyleCallback pattern and 
a `styleCallback(...)` to dynamically listen for changes in the values of all inheritable CSS
on its host element. Whenever these values changes, the IframeInheritsStyleProperties sends a message to the
...
<script>(function(){
  ...
  class IframeInheritsStyleProperties extends StyleCallbackMixin(HTMLElement) {
    constructor() {
      super();
...
//      }
  ...
  customElements.define("iframe-inherits-style", IframeInheritsStyleProperties);
